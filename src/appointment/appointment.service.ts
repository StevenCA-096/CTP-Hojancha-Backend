...
export class AppointmentService {
  ...
  async setNotAvailableAppointment(id: number) {
    const appointmentFound = await this.appointmentRepository.findOneBy({ id });
    if (!appointmentFound) throw new HttpException(`Appointment with ID ${id} not found`, HttpStatus.NOT_FOUND);

    if (appointmentFound.taken == false) {
      try {
        appointmentFound.taken = true;
        return this.appointmentRepository.save(appointmentFound)
      } catch (error) {
        return error;
      }
    } else {
      throw new HttpException(`Appointment with ID ${id} is taken`, HttpStatus.CONFLICT)
    }
  }

  async setAvailableAppointment(id: number) {
    const appointmentFound = await this.appointmentRepository.findOneBy({ id });
    if (!appointmentFound) throw new HttpException(`Appointment with ID ${id} not found`, HttpStatus.NOT_FOUND);

    try {
      appointmentFound.taken = false;
      return this.appointmentRepository.save(appointmentFound)
    } catch (error) {
      return error;
    }
